---
title: 'Voice Calls'
description: 'Integrate Rannah رنّة voice calls using the @rannah-labs/web-sdk package.'
icon: 'microphone'
---

## Overview

The **@rannah-labs/web-sdk** package enables voice call functionality via WebRTC. It allows you to initialize a voice call session with an agent (identified by `agentId` and `region` from your [Rannah رنّة Dashboard](https://rannah.io) and provides event listeners to track call status.
...
const VoiceCallPage = () => {
  ...
  async function initVoice() {
    ...
    await voice.init({
      agentId: "", // Replace with a valid agentId from your Rannah رنّة Dashboard URL (e.g., "enit5lczmqbz1s7d")
...
<CardGroup cols={2}>
  <Card title="NPM" icon="npm" href="https://www.npmjs.com/package/@rannah-labs/web-sdk">
    `@rannah-labs/web-sdk` on npm
  </Card>
