---
title: 'WordPress'
description: 'Integrate your Rannah رنّة Agent into your WordPress website.'
icon: wordpress
---
...
<CardGroup cols={2}>
  <Card title="" icon="face-smile-wink">
    A Rannah رنّة account with a configured AI agent
  </Card>
...
There are three main ways to integrate your Rannah رنّة Agent widget into WordPress:
...
## Rannah رنّة Agent Widget Code

Before proceeding with any method, you'll need your Rannah رنّة Agent widget code:
...
<Warning>
  Remember to replace **YOUR_AGENT_ID** with your actual Rannah رنّة Agent ID.
</Warning>
...
<Steps>
  ...
  <Step title="Add Widget Code">
    Paste the Rannah رنّة Agent widget code in the header section
  </Step>
...
<Steps>
  ...
  <Step title="Add Widget Code">
    Paste the Rannah رنّة widget code just before the closing `</head>` tag
  </Step>
  <Step title="Save Changes">
...
<Steps>
  ...
  <Step title="Configure Custom Code">
    ...
    4. Paste the Rannah رنّة widget code in the code block editor
  </Step>
...
You can customize your widget's behavior and appearance by adjusting the RANNAH_CONFIG options:
...
<AccordionGroup>
  ...
  <Accordion title="Autostart Chat">
    Add `autostart: true,` to the `RANNAH_CONFIG` object to have the widget open automatically when a user visits your website.
  </Accordion>
...
## Troubleshooting
**If you encounter issues with your Rannah رنّة widget integration:**

1. Verify that you've copied the entire code snippet correctly.
2. Double-check that you've replaced "YOUR_AGENT_ID" with your actual Rannah رنّة Agent ID.
...
<Note>
  If you're using a security plugin, make sure it's not blocking the Rannah رنّة scripts from loading.
</Note>