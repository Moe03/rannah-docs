...
<CardGroup>
  ...
  <Card title="Input Methods" icon="microphone">
    ...
    - **Enable Quick Upload Button** (`enableQuickFileUpload`) - Shows a paperclip attachment button in the web widget footer. Visitors can attach images and files before sending; images appear as visual messages in the conversation. Uploads from embedded widgets use the secure public upload API (no Firebase login required on the visitor's site). Voiceflow agents receive file URLs via intent triggers when configured.
  </Card>
</CardGroup>
