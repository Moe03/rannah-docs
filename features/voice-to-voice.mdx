...
In the era of advanced AI and Large Language Models (LLMs), voice interactions have reached unprecedented levels of naturalness. Rannah رنّة Agent harnesses this technology by integrating with VAPI, a cutting-edge voice-to-voice platform, to provide your agents with lifelike conversational abilities.
...
This guide will walk you through setting up your VAPI profile and enabling voice-to-voice capabilities for your Rannah رنّة Agent.
...
<Steps>
  ...
  <Step title="Configure Rannah رنّة Agent Credentials">
    **In Rannah رنّة Dashboard:**
    1. Go to the `credentials tab`
    2. Paste your VAPI private and public keys in the appropriate fields
    3. For the Default Server URL secret, go back to VAPI, find it under settings > `Server URL`, and paste it in Rannah رنّة's credentials tab
...
Now that you've set up VAPI, it's time to enable voice capabilities for your Rannah رنّة Agent:
...
<Steps>
  ...
  <Step title="Congrats" icon="party-horn">
    You've successfully set up voice-to-voice capabilities in Rannah رنّة, giving users a more natural way to interact with your brand.
...
<AccordionGroup>
  <Accordion title="Sync KB with VAPI">
    **This option adds your agent's entire Knowledge Base (KB) to your VAPI assistant.**
    ...
    - Rannah رنّة will inject your KB documents into the VAPI assistant whenever changes are made to the KB
  ...
  <Accordion title="Sync all on save">
    **Enable this to automatically update your VAPI assistant with changes made to your Rannah رنّة Agent, including:**
    - Initial message
  ...
  <Accordion title="Enable VAPI on Web">
    When enabled (`vapiConfig.enableVapiOnWeb`), the web widget footer shows a microphone button that opens the VAPI voice interface for in-browser calls. Requires a configured VAPI assistant (`vapi.id`).

    **You can customize the VAPI Web popup message in the provided input field.**

    <Note>
    This setting is separate from **Enable Quick Upload Button** (`enableQuickFileUpload`), which controls the paperclip file/image attachment button. See [Agent Settings](/agent-creation/settings) for upload configuration.
    </Note>
  </Accordion>
