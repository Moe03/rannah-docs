---
title: 'Crawler'
description: 'Efficiently build your agents knowledge base with the Rannah رنّة AI Crawler'
icon: spider
---
...
## Schema / AI Listing crawl + Structured Search

In addition to classic markdown crawls (for Knowledge Base documents), you can run a **Schema / AI Listing** crawl:

1. Turn on **Schema listing crawl (AI)** in New Job.
2. Paste **one** listing/index URL — the page that shows the full catalog (grid/list of cards). Multiple source URLs are **not** supported in this mode.
   - Example: `https://www.example.com/venues` (or `/cars`, `/properties`, `/products`).
   - Do **not** paste a single detail page.
3. The crawler discovers detail pages from that index, designs a schema, and extracts structured JSON items.
4. On the job page, use **Structured Search** → **Index** → **Add to agents**.

Agents can then filter catalogs (location, capacity, price, amenities…) and render carousels/cards with real gallery images.

<Card title="Full Structured Search guide" icon="magnifying-glass-chart" href="/features/structured-search">
  Step-by-step workflow, KB vs Structured Search, limits, and tips for production use.
</Card>

<Tip>
  In the dashboard, open **Crawler Jobs → How to use** for an in-app walkthrough of both crawl modes.
</Tip>

## How the Crawler Works

<Info>
  The crawler operates by systematically visiting web pages, extracting relevant content, and organizing it into markdown suitable for your agent's knowledge base — or into structured JSON for Structured Search catalogs. This process involves crawling through links, scraping text (or listing fields), and formatting the data for optimal use by AI models.
</Info>

## Crawler Jobs

The core of the crawler functionality revolves around crawler jobs. Each job is associated with specific source URLs you want to crawl and is identified by a unique ID. The [developer API](/api-reference/v3/crawler/createJob) offers several ways to interact with the crawler.
...
<Steps>
  <Step title="Set Source URLs">
    Navigate to the crawler tab from the menu on the left side of your dashboard. Click on `new job` and Enter the main URL(s) you want the crawler to begin with (e.g., https://rannah.io).
    <Warning>
...
### Available Refresh Rate Options:
  ...
  <Step title="Specify Page Limit">
    ...
    <Tip>
      Review the **sitemap** beforehand to determine the optimal number of pages to scrape. To view, write `/sitemap.xml` at the end of a valid URL. Ex. https://rannah.io/sitemap.xml or use [this](https://www.seowl.co/sitemap-extractor/)
    </Tip>
  </Step>
...
```markdown
Rannah رنّة provides access to a wide range of **state-of-the-art** AI models, ensuring that your agents are always equipped with the best and newest models on the market.

=========================================================

As soon as **new models** are released, the Rannah رنّة AI team promptly updates the platform. This means you typically get access to the latest and most powerful models **right away**.
```
<Note>
