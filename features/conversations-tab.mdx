...
<Step title="ID" icon="id-card">
 Unique identifier for each user (auto-generated or set via the [API](/api-reference/v3/intro))
</Step>
...
<Warning>
If you hit an API rate limit, wait for the time shown in the error message (or the `Retry-After` header) before retrying. For large archives, export in batches or use the [Conversations API](/api-reference/v3/conversations/export).
</Warning>

<Tip>
Need programmatic exports? See [Export Agent Conversations (V3 API)](/api-reference/v3/conversations/export) and [Export Convos (V2 API)](/api-reference/agents/convos/export).
</Tip>
